function facegrad=InterpolateGrad(Mesh,Field,Type)
    ...
    switch Type
        ...
        case 'scalar1'
            facegrad=zeros(Mesh.face.number,3);
            %Interior faces
            facegrad(interiorface,:)=Mesh.face.gcf(interiorface,1).*Field.element.scalar1grad(Mesh.face.owner(interiorface,1),:)...
                                    +Mesh.face.gcf(interiorface,2).*Field.element.scalar1grad(Mesh.face.owner(interiorface,2),:);      %Linear interpolation
            facegrad(interiorface,:)=facegrad(interiorface,:)...
                                    +(...
                                        (Field.element.scalar1(Mesh.face.owner(interiorface,2))-Field.element.scalar1(Mesh.face.owner(interiorface,1)))./Mesh.face.dCF(interiorface)...
                                        -sum(facegrad(interiorface,:).*Mesh.face.ecf(interiorface,:),2)...
                                     ).*Mesh.face.ecf(interiorface,:);
            %Boundary faces
            facegrad(boundaryface,:)=Field.element.scalar1grad(Mesh.face.owner(boundaryface,1),:)...
                                    +(...
                                      (...
                                       Field.face.scalar1(boundaryface)-Field.element.scalar1(Mesh.face.owner(boundaryface,1))...
                                      )./Mesh.face.dCF(boundaryface)...
                                      -sum(Field.element.scalar1grad(Mesh.face.owner(boundaryface,1),:).*Mesh.face.ecf(boundaryface,:),2)...
                                     )...
                                     .*Mesh.face.ecf(boundaryface,:);
    end
end